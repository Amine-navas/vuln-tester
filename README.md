...
### Machine Learning Malware Detection Platform
...
**Malware Detector** is a lightweight cybersecurity platform designed to detect potentially malicious samples using **Machine Learning**.
...
## 🖼️ Live Example — Working App

Below are real outputs from the web interface (`évaluez le risque`), showing the detection engine classifying two different files.
...
<img src="img/screenshot-malicious.png" alt="Malware Detector Screenshot" width="80%"/>
...
<img src="img/screenshot-benign.png" alt="Malware Detector Screenshot" width="80%"/>
...
│   │   └── random_forest.pkl
│   │
│   └── data.db                # generated locally, not versioned (see .gitignore)
│
├── database/
│   └── schema.sql
│
...
`ml/train.py` reads the SQL schema and relevant data from `database/schema.sql`, and can construct a local SQLite database at `ml/data.db`. This design allows local training without requiring a permanent MySQL/MariaDB server.
...
The default classifier is `RandomForestClassifier`.
...
The trained model is persisted under `ml/models/random_forest.pkl`.
...
The detection service loads the trained model and generates predictions:
...
The risk-analysis layer then interprets the prediction into a higher-level security assessment:
...
> The exact response depends on the implementation and the currently loaded model.
...
Returns the status of the latest training job — process ID, running state, logs, job status.
...
The Flask development server listens on `http://localhost:5000`.
...
1. Reads the SQL dataset
2. Builds the local SQLite database
3. Prepares the feature matrix
4. Splits the dataset
5. Trains the classifier
6. Evaluates the model
7. Serializes the trained model

The resulting model is stored at `ml/models/random_forest.pkl`.
...
For production deployments, consider a more robust logging architecture: Python `logging`, JSON structured logs, log rotation, centralized log collection, metrics, health checks, monitoring dashboards.
...
| Layer                 | Purpose                                             |
| --------------------- | --------------------------------------------------- |
| `database/schema.sql` | Original SQL-based dataset / schema                 |
| `ml/data.db`          | SQLite database generated locally for ML processing |
...
**🧠 Machine Learning** — XGBoost, Gradient Boosting, SVM, Logistic Regression, Neural Networks, Ensemble classifiers, Scikit-learn pipelines

**🔬 Feature Engineering** — PE header characteristics, section statistics, import/export info, entropy, string characteristics, file metadata, opcode-derived features, API-call statistics, behavioral indicators

**🌐 API** — Authentication, API keys, rate limiting, OpenAPI/Swagger docs, versioned endpoints, async job queues
...
| ------------------------ | ------------------------------------------------------------------------------- |
| 🧱 Modularity            | Application, ML and data-processing components remain separated                 |
| 🔍 Auditability          | The codebase keeps the detection pipeline understandable and inspectable        |
| ⚡ Lightweight execution | The project avoids unnecessary dependencies where practical                     |
| 🔄 Reproducibility       | Training scripts and explicit dependencies make experiments easier to reproduce |
| 🔐 Security by design    | Sensitive operations such as model training are protected before production     |
