...
if ($LASTEXITCODE -ne 0) {
    ...
    Invoke-Checked -Executable $venvPython -Arguments @('-m', 'pip', '--disable-pip-version-check', 'install', '-r', $requirementsFile) -Description 'Dependency installation'
}
...
if (-not (Test-Server)) {
    ...
    Start-Process -WindowStyle Hidden -FilePath $venvPython -ArgumentList '-m', 'app.app' -WorkingDirectory $projectRoot -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
...
Invoke-Checked -Executable $venvPython -Arguments @('-m', 'pytest', '-q', 'tests') -Description 'Test suite'
